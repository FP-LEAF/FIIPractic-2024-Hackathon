export default class OrgChart extends HTMLElement {
    ...
    if (opts.toggleSiblingsResp && (typeof opts.ajaxURL === 'undefined' ||
      ...
      if (prevSib) {
        if (prevSib.classList.contains('hidden')) {
          ...
          leftEdge.classList.add('pe-7s-angle-left');
          leftEdge.classList.remove('pe-7s-angle-right');
        } else {
          // leftEdge.classList.add('fa-chevron-right');
          // leftEdge.classList.remove('fa-chevron-left');
          leftEdge.classList.add('pe-7s-angle-right');
          leftEdge.classList.remove('pe-7s-angle-left');
        }
      }
      if (nextSib) {
        if (nextSib.classList.contains('hidden')) {
          ...
          rightEdge.classList.add('pe-7s-angle-right');
          rightEdge.classList.remove('pe-7s-angle-left');
        } else {
          // rightEdge.classList.add('fa-chevron-left');
          // rightEdge.classList.remove('fa-chevron-right');
          rightEdge.classList.add('pe-7s-angle-left');
          rightEdge.classList.remove('pe-7s-angle-right');
        }
      }
    } else {
      ...
      leftEdge.classList.toggle('pe-7s-angle-right', sibsVisible);
      leftEdge.classList.toggle('pe-7s-angle-left', !sibsVisible);
      rightEdge.classList.toggle('pe-7s-angle-left', sibsVisible);
      rightEdge.classList.toggle('pe-7s-angle-right', !sibsVisible);
    }
  }
  _hoverNode(event) {
    ...
    if (event.type === 'mouseenter') {
      if (topEdge) {
        ...
        topEdge.classList.toggle('pe-7s-angle-up', !flag);
        topEdge.classList.toggle('pe-7s-angle-down', flag);
      }
      if (bottomEdge) {
        ...
        bottomEdge.classList.toggle('pe-7s-angle-down', !flag);
        bottomEdge.classList.toggle('pe-7s-angle-up', flag);
      }
      if (leftEdge) {
      ...
      Array.from(node.querySelectorAll(':scope > .edge')).forEach((el) => {
        ...
        el.classList.remove('pe-7s-angle-up', 'pe-7s-angle-down', 'pe-7s-angle-right', 'pe-7s-angle-left');
      });
    }
  }
  ...
  _switchVerticalArrow(arrow) {
    ...
    arrow.classList.toggle('pe-7s-angle-up');
    arrow.classList.toggle('pe-7s-angle-down');
  }
  ...
  showSiblings(node, direction) {
    ...
    this._one(siblings[0].querySelector('.slide'), 'transitionend', function () {
      ...
      if (this._isInAction(node)) {
        ...
        node.querySelector('.topEdge').classList.remove('pe-7s-angle-up');
        node.querySelector('.topEdge').classList.add('pe-7s-angle-down');
      }
    }, this);
  }
  ...
  addParent(currentRoot, data) {
    ...
    this._buildParentNode(currentRoot, data, function () {
      if (!currentRoot.querySelector(':scope > .topEdge')) {
        ...
        topEdge.setAttribute('class', 'edge verticalEdge topEdge');
    ...
    this._buildChildNode.call(this, this._closest(node, (el) => el.nodeName === 'TABLE'), data, function () {
      if (++count === data.children.length) {
        if (!node.querySelector('.bottomEdge')) {
          ...
          bottomEdge.setAttribute('class', 'edge verticalEdge bottomEdge');
        ...
        if (!node.querySelector('.symbol')) {
          ...
          symbol.setAttribute('class', opts.parentNodeSymbol + ' symbol');
  ...
  addSiblings(node, data) {
    ...
    this._buildSiblingNode.call(this, this._closest(node, (el) => el.nodeName === 'TABLE'), data, () => {
      ...
      if (!node.querySelector('.leftEdge')) {
        ...
        rightEdge.setAttribute('class', 'edge horizontalEdge rightEdge');
        node.appendChild(rightEdge);
        // leftEdge.setAttribute('class', 'edge horizontalEdge leftEdge fa');
        leftEdge.setAttribute('class', 'edge horizontalEdge leftEdge');
