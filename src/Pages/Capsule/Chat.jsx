...
const Chat = () => {
    ...
    return (
        <div className="fixed bottom-5 right-5 z-50">
            <button onClick={toggleChat} className="px-4 py-2 bg-pink-500 text-white font-medium rounded-lg shadow hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
                Chat with A.I
            </button>
            {isOpen && (
                    ...
                    <div className="overflow-y-auto h-[32rem] mb-4 space-y-6">
                        {messages.map((msg, index) => (
                            <div key={index} className={`flex items-end ${msg.sender === 'User' ? 'justify-end' : 'justify-start'}`}>
                                {msg.sender !== 'User' && <img className="w-8 h-8 rounded-full" src="https://cuad.ask.fm/assets2/070/462/562/304/normal/image1.jpg" alt="Bot image" />}
                                <div className={`flex flex-col w-full max-w-[320px] leading-1.5 p-4 ${msg.sender === 'User' ? 'bg-white-500 rounded-br-none' : 'bg-gray-100 rounded-bl-none'} border border-gray-200 dark:bg-gray-700`}>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-semibold text-gray-900 dark:text-white">{msg.sender === 'User' ? 'You' : 'Bot'}</span>
                                    ...
                                    {msg.sender === 'User' && <span className="text-sm font-normal text-gray-500 dark:text-gray-400">Delivered</span>}
                                </div>
                                {msg.sender === 'User' && <img className="w-8 h-8 rounded-full" src="https://flowbite.com/docs/images/people/profile-picture-5.jpg" alt="User image" />}
                                <div ref={messagesEndRef} /> {/* Add this line. This is a dummy element at the end of your messages */}
                            </div>
                        ))}
                    </div>
